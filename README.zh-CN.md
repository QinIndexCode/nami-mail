...
<p align="center">
  <img src="docs/nami-mail-inbox-zh.png" alt="Nami Mail 收件箱界面" width="1200" />
</p>

<p align="center">
  <em>多账户统一收件箱、文件夹导航与阅读窗格。</em>
</p>

<p align="center">
  <img src="docs/nami-mail-agent-zh.png" alt="Nami Mail Agent 工作区" width="1200" />
</p>

<p align="center">
  <em>NamiMail Agent：本地优先的邮件 AI 助手，带来源引用。</em>
</p>
...
- **External Mail v1**：面向已配对本机脚本和 AI 助手的配对、本机 CLI 与 MCP 接口。它通过仅当前 Windows 用户可访问的命名管道 Broker 暴露 16 项工具——9 个只读（账户、文件夹、邮件列表/搜索/摘要、单条与批量详情、线程、附件元数据）+ 7 个受限写入（草稿创建/更新/删除、移动、标记、发送、回复）；默认 `read-only`。
...
首次安装、Windows SmartScreen、同版本重装、降级保护、数据保留、卸载与自动更新的用户路径见 [Windows 安装与更新指南](docs/INSTALLING.zh-CN.md)。邮箱的认证准备、服务商差异、OAuth 前置条件和手动配置说明见 [邮箱接入指南](docs/EMAIL-PROVIDERS.zh-CN.md)。

## 文档

- [文档导航](docs/README.md)：按使用、贡献和发布任务查找中英文文档。
- [Windows 安装与更新指南](docs/INSTALLING.zh-CN.md)：下载、安装、首次启动、卸载、更新提示和 SmartScreen 风险说明。
- [邮箱接入指南](docs/EMAIL-PROVIDERS.zh-CN.md)：服务商认证准备、OAuth、手动 IMAP/SMTP 和常见连接问题。
- [外部 Mail 接口](docs/EXTERNAL-MAIL-INTERFACE.zh-CN.md)：供已配对本机脚本和 MCP 客户端使用的只读 CLI/MCP 接口与安全边界。
- [CLI 参考](docs/cli/README.zh-CN.md)：`namimail` CLI 的命令、参数、输出格式和示例。
- [MCP Server](docs/mcp/README.zh-CN.md)：本机 MCP stdio 接入、工具发现和安全边界。
- [NamiMail Agent 使用指南](docs/agent/usage.zh-CN.md)：对话管理、邮件范围、来源引用与确认流程。
- [NamiMail Agent 模型提供商配置](docs/agent/providers.zh-CN.md)：OpenAI 兼容/Ollama/Claude/Gemini 模型、密钥与云端邮件内容授权。
- [NamiMail Agent 接入外部 MCP 服务器](docs/agent/mcp-servers.zh-CN.md)：接入外部 MCP 服务器扩展助理工具。
- [NamiMail Agent 架构](docs/agent/architecture.zh-CN.md)：本地优先的 Agent 设计、嵌入式工作区与外部 Broker 边界。
- [邮件 RAG](docs/rag/architecture.zh-CN.md)：邮件入库、清洗、检索、删除同步与一致性说明。
- [邮件正文翻译](docs/TRANSLATION.zh-CN.md)：可选翻译服务的配置、主动发送边界和隐私注意事项。
- [隐私与本地数据说明](docs/PRIVACY.zh-CN.md)：本地保存内容、加密边界和第三方连接。
- [支持指南](SUPPORT.zh-CN.md)：邮箱接入、网络问题和适合提交 Issue 的内容。
- [安全策略](SECURITY.zh-CN.md)：私下报告漏洞的方式和范围。
- [贡献指南](CONTRIBUTING.zh-CN.md)、[社区行为准则](CODE_OF_CONDUCT.zh-CN.md)、[开发说明](docs/DEVELOPMENT.zh-CN.md) 与 [架构与信任边界](docs/ARCHITECTURE.zh-CN.md)：本地开发、协作边界、测试、进程边界和 Pull Request 要求。
- [Windows 发布指南](docs/RELEASING.zh-CN.md)、[Release Notes](docs/releases/README.zh-CN.md) 与 [变更日志](CHANGELOG.zh-CN.md)：签名、GitHub Release、面向用户的版本说明和发行验证。
- [本地化说明](docs/LOCALIZATION.zh-CN.md)：界面语言包与文档翻译的维护约定。
...
普通用户应使用 Release 页面列出的 Windows `.exe` 安装程序，不要把更新 ZIP 解压后直接运行。安装前和遇到 Windows 信任提示时，请按 [Windows 安装与更新指南](docs/INSTALLING.zh-CN.md) 核对来源与签名状态。
...
- 后台周期同步和手动同步
- 首次连接即同步全部可选文件夹；默认每个文件夹缓存最近 2000 封，可在设置 → 同步中调整为 500/1000/5000 或「全部」（与 Gmail 网页版一致），环境变量 `SYNC_MESSAGE_LIMIT` 显式设置时覆盖应用内设置
...
- **NamiMail Agent**：AI 助手，支持本地检索、来源引用和草稿管理，在明确的账户范围内工作
- **邮件翻译**：免费翻译（Google/MyMemory）和可选的 AI 翻译，支持流式输出与取消
- **External Mail v1**：面向已配对本机脚本和 AI 助手的只读 CLI 和 MCP stdio 接口
...
- 首次启动显示覆盖整个应用的使用条款与隐私协议弹窗，明确同意后才存储任何数据
