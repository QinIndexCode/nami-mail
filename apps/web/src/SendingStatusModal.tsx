...
import { CalendarClock, Check, CircleAlert, LoaderCircle, Mail, PenLine, RefreshCw, Send, X } from "lucide-react";
...
export default function SendingStatusModal({
  ...
  const [filter, setFilter] = useState<"all" | "active" | "attention" | "confirmed">("all");
  ...
  useEffect(() => {
    const handleEscape = (event: KeyboardEvent) => {
      ...
      if (confirmReplacement) requestConfirmClose();
      else requestClose();
    };
    window.addEventListener("keydown", handleEscape, true);
    return () => window.removeEventListener("keydown", handleEscape, true);
  }, [confirmReplacement, requestClose, requestConfirmClose]);
  ...
  return (
    <>
      <div className={`modal-backdrop sending-status-backdrop${closing ? " closing" : ""}`} role="presentation" onMouseDown={(event) => {
        if (event.target === event.currentTarget && !confirmReplacement) requestClose();
      }}>
        <section ref={dialogRef} className={`modal-card sending-status-modal${closing ? " closing" : ""}`} role="dialog" aria-modal="true" aria-labelledby="sending-status-title" tabIndex={-1}>
            <header className="modal-heading sending-status-heading">
              ...
              <div className="sending-status-heading-actions">
                <StatusIconButton label={t("sending.modal.refreshTooltip")} onClick={() => void refresh()} disabled={loading}>
                  {loading ? <LoaderCircle className="spin" size={17} /> : <RefreshCw size={17} />}
                </StatusIconButton>
                <StatusIconButton label={t("sending.modal.closeTooltip")} onClick={requestClose}><X size={18} /></StatusIconButton>
              </div>
            </header>

            <div className="sending-status-overview" role="tablist" aria-label={t("sending.modal.overviewLabel")}>
              <button type="button" role="tab" aria-selected={filter === "all"} className={filter === "all" ? "active" : ""} onClick={() => setFilter("all")}><span>{t("sending.modal.all")}</span><strong>{counts.all}</strong></button>
              <button type="button" role="tab" aria-selected={filter === "active"} className={filter === "active" ? "active" : ""} onClick={() => setFilter("active")}><span>{t("sending.modal.active")}</span><strong>{counts.active}</strong></button>
              <button type="button" role="tab" aria-selected={filter === "attention"} className={`${filter === "attention" ? "active" : ""}${counts.attention ? " attention" : ""}`} onClick={() => setFilter("attention")}><span>{t("sending.modal.attention")}</span><strong>{counts.attention}</strong></button>
              <button type="button" role="tab" aria-selected={filter === "confirmed"} className={filter === "confirmed" ? "active" : ""} onClick={() => setFilter("confirmed")}><span>{t("sending.modal.confirmed")}</span><strong>{counts.confirmed}</strong></button>
            </div>
            ...
            <div className="sending-status-list" role="list" aria-label={t("sending.modal.listLabel")} aria-live="polite" aria-busy={loading}>
              ...
              {visibleSubmissions.map((submission) => {
                ...
                const isConfirmed = submission.deliveryStatus === "confirmed";
                ...
                return (
                  <article className={`sending-status-item tone-${presentation.tone}${isConfirmed ? " sending-status-item-compact" : ""}`} key={submission.id} role="listitem">
                    ...
                    {!isConfirmed && <p className="sending-status-detail">{presentation.detail}</p>}
                    {isPendingScheduled && submission.sendAt && (
                      <p className="sending-status-scheduled-time"><CalendarClock size={13} />{t("sending.modal.scheduledTime", { time: formatSubmissionTime(submission.sendAt, formatDate, t) })}</p>
                    )}
                    {statusMessage && (
                      <p className="sending-status-message">{statusMessage}</p>
                    )}
                    {actionError && (
                      <p className="sending-status-action-error" role="alert"><CircleAlert size={13} />{actionError}</p>
                    )}
                    {!isConfirmed && (
                    ...
                    {!isConfirmed && (canSync || canCreate || isPendingScheduled) && (
