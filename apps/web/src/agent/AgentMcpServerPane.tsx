    ...
    <>
      <section className="agent-provider-catalog" aria-label={t("agent.mcpServers.title")}>
          ...
          <form className="agent-provider-form" onSubmit={(event) => { event.preventDefault(); void saveServer(); }}>
            ...
            <div className="agent-provider-field"><span><strong>{t("agent.mcpServers.fields.env")}</strong><button className="agent-provider-help" type="button" aria-label={t("agent.mcpServers.fields.envHint")} data-tooltip={t("agent.mcpServers.fields.envHint")}><CircleHelp size={12} /></button></span>
            ...
            <label className="agent-provider-field agent-provider-timeout"><span><strong>{t("agent.mcpServers.fields.timeout")}</strong><button className="agent-provider-help" type="button" aria-label={t("agent.mcpServers.fields.timeoutHint")} data-tooltip={t("agent.mcpServers.fields.timeoutHint")}><CircleHelp size={12} /></button></span><input type="text" inputMode="numeric" pattern="[0-9]*" value={form.timeoutMs} disabled={busy} onChange={(event) => updateForm("timeoutMs", event.target.value)} autoComplete="off" /></label>
            <button className={`agent-provider-toggle-row ${form.enabled ? "active" : ""}`} type="button" role="switch" aria-checked={form.enabled} disabled={busy} onClick={() => updateForm("enabled", !form.enabled)}><span><strong>{t("agent.mcpServers.fields.enabled")}</strong><button className="agent-provider-help" type="button" aria-label={t("agent.mcpServers.fields.enabledHint")} data-tooltip={t("agent.mcpServers.fields.enabledHint")}><CircleHelp size={12} /></button></span><span className="agent-provider-switch" aria-hidden="true"><span /></span></button>
