    ...
    >
      <aside ref={dialogRef} className={`agent-provider-settings${closing ? " closing" : ""}`} role="dialog" aria-modal="true" aria-label={t("agent.providers.title")} tabIndex={-1}>
        <header className="agent-provider-settings-header">
          <div><span className="eyebrow">NAMI AGENT</span><span className="agent-provider-title-line"><h2>{pane === "mcp" ? t("agent.mcpServers.title") : t("agent.providers.title")}</h2><button className="agent-provider-help" type="button" aria-label={pane === "mcp" ? t("agent.mcpServers.description") : t("agent.providers.description")} data-tooltip={pane === "mcp" ? t("agent.mcpServers.description") : t("agent.providers.description")}><CircleHelp size={13} /></button></span></div>
...
<label className="agent-provider-field agent-provider-timeout"><span><strong>{t("agent.providers.fields.timeout")}</strong><button className="agent-provider-help" type="button" aria-label={t("agent.providers.fields.timeoutHint")} data-tooltip={t("agent.providers.fields.timeoutHint")}><CircleHelp size={12} /></button></span><input type="text" inputMode="numeric" pattern="[0-9]*" value={form.timeoutMs} disabled={saving} onChange={(event) => updateForm("timeoutMs", event.target.value)} autoComplete="off" /></label>
            ...
            <button className={`agent-provider-toggle-row ${form.makeDefault ? "active" : ""}`} type="button" role="switch" aria-checked={form.makeDefault} disabled={saving || isDefaultProvider} onClick={() => updateForm("makeDefault", !form.makeDefault)}><span><strong>{t("agent.providers.default.title")}</strong><button className="agent-provider-help" type="button" aria-label={isDefaultProvider ? t("agent.providers.default.current") : t("agent.providers.default.description")} data-tooltip={isDefaultProvider ? t("agent.providers.default.current") : t("agent.providers.default.description")}><CircleHelp size={12} /></button></span><span className="agent-provider-switch" aria-hidden="true"><span /></span></button>
