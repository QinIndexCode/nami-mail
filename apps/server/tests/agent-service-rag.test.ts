...
describe("Agent service lifecycle fence", () => {
  ...
  it("never returns the assistant reply twice during the teardown window", async () => {
    ...
    const internals = service as unknown as {
      ...
      readConversation: (id: string) => { leases: unknown[] };
    };
    ...
    internals.conversations.append(conversation.id, internals.readConversation(conversation.id).leases, "turn", {
...
describe("Agent service streaming persistence", () => {
  let db: DatabaseHandle | undefined;
  let masterKey: Buffer | undefined;
  ...
  it("exposes the partial reply as streaming mid-run and replaces it with the final copy", async () => {
    db = openDatabase(":memory:");
    masterKey = randomBytes(32);
    insertAccount(db);
    applyAgentStoreSchema(db, "2026-07-27T10:00:00.000Z");
    ...
    const internals = service as unknown as {
      rag: AgentRagWorker;
      runtime: { streamChat: () => AsyncIterable<unknown> };
    };
    vi.spyOn(internals.rag, "search").mockResolvedValue([]);
    ...
    while (true) {
      ...
      if (next.done) break;
      events.push(next.value);
      if (next.value.type === "text_delta" && next.value.delta === "First half. ") {
        // The server snapshots in-flight state *after* yielding each delta, so
        // advance one more event to let the first delta's syncInFlight land
        // before checking the live conversation mid-run.
    ...
    const during = service.getConversation(conversation.id);
    const streaming = during.messages[during.messages.length - 1];
    expect(streaming?.role).toBe("assistant");
    expect(streaming?.state).toBe("streaming");
    expect(streaming?.content).toBe("First half. ");
    ...
    const after = service.getConversation(conversation.id);
    const assistants = after.messages.filter((message) => message.role === "assistant");
    expect(assistants).toHaveLength(1);
    expect(assistants[0]?.state).toBe("complete");
    expect(assistants[0]?.content).toBe("First half. Second half.");
    await service.close();
  });

  it("turns an orphaned persisted streaming row into an interrupted error", async () => {
    db = openDatabase(":memory:");
    masterKey = randomBytes(32);
    insertAccount(db);
    applyAgentStoreSchema(db, "2026-07-27T10:00:00.000Z");
    ...
    const internals = service as unknown as {
      conversations: { upsertStreaming: (id: string, leases: unknown[], value: unknown, messageId: string) => void };
      readConversation: (id: string) => { leases: unknown[] };
    };
    // Simulate a process that vanished mid-stream: only the partial draft
    // exists in storage and no run is alive to finish it.
    const state = internals.readConversation(conversation.id);
    internals.conversations.upsertStreaming(conversation.id, state.leases, {
      type: "conversation-turn",
      message: {
        id: "orphan-1",
        role: "assistant",
    ...
    expect(last?.id).toBe("orphan-1");
    expect(last?.state).toBe("error");
    expect(last?.error?.code).toBe("INTERRUPTED");
    expect(last?.content).toBe("Stale partial");
    await service.close();
  });

  it("keeps the partial reply in the transcript and next-turn context when interrupted", async () => {
    db = openDatabase(":memory:");
    masterKey = randomBytes(32);
    insertAccount(db);
    applyAgentStoreSchema(db, "2026-07-27T10:00:00.000Z");
    ...
    const internals = service as unknown as {
      rag: AgentRagWorker;
      runtime: { streamChat: (request: { signal?: AbortSignal; chat: { messages: unknown[] } }) => AsyncIterable<unknown> };
    };
    vi.spyOn(internals.rag, "search").mockResolvedValue([]);
    ...
    vi.spyOn(internals.runtime, "streamChat").mockImplementation(async function* (request) {
      ...
      // A real provider rejects on abort; without the throw the turn would end
      // "cleanly" instead of being classified as cancelled.
      await new Promise<void>((_resolve, reject) => {
        const onAbort = () => reject(Object.assign(new Error("aborted"), { code: "CANCELLED" }));
    ...
    // The partial reply is persisted as "interrupted", not dropped. The
    // persisted copy is trimmed of trailing whitespace (memory-suggestion
    // stripping), matching every other stored turn.
    const view = service.getConversation(conversation.id);
    const last = view.messages[view.messages.length - 1];
    ...
    // The next turn's provider request includes that partial reply.
    let secondMessages: unknown[] = [];
    vi.spyOn(internals.runtime, "streamChat").mockImplementation(async function* (request) {
      secondMessages = request.chat.messages;
      yield { type: "completed", reason: "stop" };
    });
    ...
    expect(secondEvents).toContainEqual({ type: "completed", reason: "stop" });
    const partialInContext = secondMessages.some((message) =>
      (message as { role?: string; content?: string }).role === "assistant"
      && (message as { content?: string }).content === "Partial answer.");
    expect(partialInContext).toBe(true);
    await service.close();
  });

  it("cancels a streaming run when its conversation is deleted", async () => {
    db = openDatabase(":memory:");
    masterKey = randomBytes(32);
    insertAccount(db);
    applyAgentStoreSchema(db, "2026-07-27T10:00:00.000Z");
    ...
    const internals = service as unknown as {
      rag: AgentRagWorker;
      runtime: { streamChat: (request: { signal?: AbortSignal }) => AsyncIterable<unknown> };
      activeRuns: Map<string, { controller: AbortController }>;
    };
    vi.spyOn(internals.rag, "search").mockResolvedValue([]);
    ...
    vi.spyOn(internals.runtime, "streamChat").mockImplementation(async function* (request) {
      ...
      // Hang until the run is aborted, like a real provider mid-reply.
      await new Promise<void>((_resolve, reject) => {
        const onAbort = () => reject(Object.assign(new Error("aborted"), { code: "CANCELLED" }));
        request.signal?.addEventListener("abort", onAbort, { once: true });
        if (request.signal?.aborted) onAbort();
    ...
    // Deleting the conversation must cancel the still-running agent instead of
    // leaving it to stream against a row that is about to vanish.
    service.deleteConversation(conversation.id);
    expect(requestSignal?.aborted).toBe(true);

    // Drain the stream: the run releases and the conversation is gone.
    ...
    expect(events).toContainEqual({ type: "completed", reason: "cancelled" });
    expect(internals.activeRuns.has(conversation.id)).toBe(false);
    expect(() => service.getConversation(conversation.id)).toThrow();
    await service.close();
  });
});
