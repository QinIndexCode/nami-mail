...
describe("Agent MCP tool registration and lifecycle", () => {
  ...
  it("unregisters tools when the server is deleted and registers none for disabled servers", async () => {
    ...
    try {
      ...
      // All MCP tools are gone; only the built-in calendar and memory tools remain.
      expect(registry.list().filter((tool) => !tool.name.startsWith("calendar.") && !tool.name.startsWith("memory."))).toHaveLength(0);
    } finally {
      await service.close();
      db.close();
    }
