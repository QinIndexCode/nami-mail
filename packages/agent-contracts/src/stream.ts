...
export const agentStreamEventSchema = z.discriminatedUnion("type", [
  ...
  streamBaseSchema.extend({ type: z.literal("reasoning_delta"), delta: z.string().min(1).max(200_000) }).strict(),
  ...
  streamBaseSchema.extend({ type: z.literal("memory_suggestion"), summary: z.string().trim().min(1).max(500) }).strict(),
