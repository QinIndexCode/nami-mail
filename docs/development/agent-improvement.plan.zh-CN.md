...
> **勘误（2026-09-13）**：第二路检索已由「向量语义检索」改为「查询扩展」（见 [`docs/rag/retrieval.*`](../rag/retrieval.zh-CN.md)）：词法臂无结果时用默认对话 Provider 生成检索词后重查同一倒排表，**不再有向量、嵌入或索引侧存储**。因此 **B-1.2（向量持久化/重建进度）作废**；B-1.1（词法索引持久化）已完成，打分已是 BM25。下面的第 2 条与第 3 条中的向量部分仅作历史记录保留。
- **现状**：
...
| **B** | ~~B-1.1 词法索引持久化~~（已完成，2026-08-12）→ C-1 全局并发 → ~~B-1.2 向量持久化/重建进度~~（2026-09-13 作废：第二路已改为查询扩展，不再有向量） | 性能 |
