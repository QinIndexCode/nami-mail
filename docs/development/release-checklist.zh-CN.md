...
[简体中文](release-checklist.zh-CN.md) | [English](release-checklist.en.md)
...
- [ ] CLI/MCP 三个权限档位 read-only/send-confirmed/full-access 生效；宿主按配置档位 clamp 外部级别，已配对客户端不能自行提权。
- [ ] `--yes`、CLI/MCP 参数或模型工具调用不能绕过确认或提权；full-access 开启前在 UI 中有明确警告。
- [ ] 外部写工具在 send-confirmed 档每次弹出可见的桌面确认，在 full-access 档自动执行。
...
参见 [测试计划](testing-plan.zh-CN.md)、[迁移计划](migration-plan.zh-CN.md) 和 [Agent 安全](../agent/security.zh-CN.md)。
