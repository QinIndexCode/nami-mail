...
先更新 [变更日志](../CHANGELOG.zh-CN.md)，确认用户可见文案、许可和版本号都正确。执行与本次改动相关的检查，再执行完整基线：
...
本仓库将可公开审阅的版本说明放在 [Release Notes 目录](releases/README.zh-CN.md)。当前 `0.2.3` 使用 [待发布说明](releases/v0.2.3.zh-CN.md)，上一版已发布版本为 [v0.1.2 说明](releases/v0.1.2.zh-CN.md)。`v0.2.2`、`v0.2.1` 与 `v0.2.0` 都仅为未完成发布验证的源码标签，没有公开 Release 或资源，不能作为安装或更新来源；其中 `v0.2.2` 的 Release 流程在创建草稿或公开 Release 前停止。保留的 [候选模板](releases/v0.1.1-candidate.zh-CN.md) 仅用于发布前检查；`v0.1.1` 仅为未发布资产的源码标签，不应作为安装来源。`0.2.3` 待发布说明故意保留“尚未完成真实线上升级验收”的安全文案，只有完成本页末尾的全部真实验证并保存证据后，才能在后续版本说明中写为已完成。
...
开发模式、普通本地安装包和没有 GitHub 更新配置或信任根的安装版必须显示为更新不可用，不能被记为已验证更新通道。发布完成后在 [README](../README.md)、[变更日志](../CHANGELOG.zh-CN.md) 和 Release Notes 中记录已验证的版本和已知限制。
