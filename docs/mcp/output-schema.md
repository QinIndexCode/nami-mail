...
[English](output-schema.en.md) | [工具](tools.md)

> **未来契约，当前不可执行。** 当前构建没有 MCP stdio Server、Broker 或 `tools/list`。下列包络只定义验证过的 Windows SID-DACL 原生适配器发布后的结构化结果，不能由当前客户端获得。

接口发布后，MCP 的外层消息将遵循客户端与 stdio Server 协商的 MCP 协议。NamiMail 将在工具结果中保留相同的稳定 Agent 成功/失败语义；支持结构化内容的客户端应把以下对象作为程序接口：
...
`code` 将是机器处理字段；`message` 和可选 `suggestion` 可被本地化。可选 `details` 不是稳定应用协议，客户端不得依赖或记录它。
