...
| `namimail_messages_move` | Moves one message to the archive, the trash, or an explicit folder of the same account. | `messageId`, plus exactly one of `target` (`"archive"` or `"trash"`) or `folder` (a folder path from `namimail_folders_list`). | `{ messageId, target }` or `{ messageId, folder }`. | `write:mail` | Desktop confirmation at `send-confirmed`; automatic at `full-access`. |
