...
| 文档 | 内容 |
| --- | --- |
| [安装](installation.zh-CN.md) | 当前不可用说明，以及未来宿主、PATH、服务启动和配对前提。 |
| [配置](configuration.zh-CN.md) | 不可粘贴的未来本地 MCP stdio 配置及版本协商。 |
| [工具](tools.zh-CN.md) | 未来 v1 只读工具名、输入/输出契约和权限。 |
| [输出 Schema](output-schema.zh-CN.md) | 未来 MCP 包装与稳定 NamiMail Agent 错误。 |
