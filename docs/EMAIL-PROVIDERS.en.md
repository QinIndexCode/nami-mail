...
| --- | --- | --- | --- |
| Gmail / Google Workspace | `gmail.com`, `googlemail.com`, enterprise custom domains | Google 16-character app password (recommended); OAuth2 optional | Recommend using a 16-character app password: enable 2-Step Verification on your Google Account, visit [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords) to create a dedicated password for Nami Mail and paste it in. If a Google Client ID is configured in your build, you can also toggle to OAuth sign-in. |
...
## Gmail Sign-in and 16-Character App Password Setup Guide

For Gmail users, Nami Mail defaults to the **16-character app password** sign-in method recommended by Google for third-party email clients. This path connects directly via SSL IMAP/SMTP without relying on external OAuth client configuration:
...
2. **Access App Passwords**: Visit the [Google App Passwords management page](https://myaccount.google.com/apppasswords).
3. **Generate a Password**: Enter `Nami Mail` as the app name and click **Create / Generate**.
4. **Copy and Paste**: A 16-character passcode will appear (e.g. `abcd efgh ijkl mnop`). Copy it and paste it directly into Nami Mail's password field to connect.

## Google and Microsoft OAuth

Nami Mail also fully implements OAuth 2.0 PKCE sign-in for both Google and Microsoft. OAuth uses a public client and a local loopback callback. It neither needs nor accepts a client secret.

- **Google**: uses a Google Cloud **Desktop app** client ID. In official releases without an embedded public client ID, switching to OAuth explains that it is not configured on this device; users can simply continue with the 16-character app password above, or developers can supply a Client ID in their environment.
- **Microsoft**: uses Microsoft Entra **Mobile and desktop applications / public client** configuration. If an organization account reports IMAP or permission restrictions, an administrator must enable the relevant capability.
- **Toggling between modes**: the interface lets you switch seamlessly between app-password sign-in and OAuth authorization.
