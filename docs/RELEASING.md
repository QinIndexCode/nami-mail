...
本仓库将可公开审阅的版本说明放在 [Release Notes 目录](releases/README.md)。当前 `0.2.1` 使用 [待发布说明](releases/v0.2.1.md)，上一版已发布版本为 [v0.1.2 说明](releases/v0.1.2.md)。`v0.2.0` 仅为未完成发布验证的源码标签，没有公开 Release 或资源，不能作为安装或更新来源。保留的 [候选模板](releases/v0.1.1-candidate.md) 仅用于发布前检查；`v0.1.1` 仅为未发布资产的源码标签，不应作为安装来源。待发布说明故意保留“尚未完成真实线上升级验收”的安全文案，只有完成本页末尾的全部真实验证并保存证据后，才能在后续版本说明中写为已完成。
