...
| `4` | Pairing, signature, or replay protection failure | `PAIRING_REQUIRED`, `PAIRING_REVOKED`, `PAIRING_EXPIRED`, `BROKER_AUTHENTICATION_FAILED`, `BROKER_REPLAY_DETECTED`, `BROKER_COUNTER_INVALID` | Pair again, revoke an invalid client, or repair the client counter. Do not replay a request. |
